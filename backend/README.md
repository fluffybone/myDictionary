...
- связанные `language_rules` и предложения по улучшению удаляются вместе с пользователем

### Предложения по улучшению

Пользователь может отправить идею через:

- `POST /api/improvement-suggestions`

А на скрытой странице можно читать список через:
